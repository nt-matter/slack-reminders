...
exports.checkForReminders = async () => {
	...
	for (const { date, name, type } of todaysReminders) {
		...
		const messages = {
			bot: `📆 <!everyone>: Un día como hoy hace ${age} ${yearText} cobraba vida el *Epic Bot* 🤖!! 🍰⌛`,
			birthday: `📆 <!everyone>: Hoy *${todayFormatted}* <${handlers[name]}> cumple ${age} ${yearText}!! 🥳🎉🎂`,
			ntm: `📆 <!everyone>: Hoy *${todayFormatted}* <${handlers[name]}> cumple ${age} ${yearText} trabajando en *Epic Devs*!! 🥳🎉`,
		}
